...
Given frozen $\hat{T}_{WB}(t)$, Stage-2 minimizes a Ceres cost over $T_{L_\ell B}$
using body-cluster centroid factors and, when the aspect gate opens, an
observed-mean lever $\mathbf{p}_B$ (Sec.~\ref{sec:mechanism}).
Each LiDAR $\ell$ is solved in an \textbf{independent}
\texttt{ExtrinsicRefiner} scope---no shared \texttt{CalibrationEstimator} state
across sensors (\texttt{multi\_lidar\_body\_calibration}, commit
\texttt{3fb7ddf}).
...
\end{equation}
Regression tests confirm production multi-LiDAR NE/SW errors match single-sensor
in-process solves (12.149\,mm / 42.041\,mm, simulation).
...
\begin{figure}[t]
  ...
  per-LiDAR Stage-2 Ceres scopes, post-hoc relative composition.
  \TODO{replace with publication-quality figure from figure\_table\_plan.md Fig.~1}.}
  \label{fig:framework}
\end{figure}

\subsection{Production modules}
Key implementations in \texttt{clic\_calib}:
\texttt{Stage1TrajectoryFitter} (PW-whitened RTK trajectory),
\texttt{body\_gated\_calibration} (gated observed-mean $\times 3$ + guards),
\texttt{observed\_mean\_gate} (yaw\_cv $\geq 0.65$ + POI path),
\texttt{multi\_lidar\_body\_calibration} (per-sensor Stage-2 loop), and
\texttt{relative\_extrinsic} (\S7.2 covariance and center-reg metric).
