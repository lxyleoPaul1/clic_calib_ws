...
/** Stage-2 body-centroid with jointly optimized p_B in body frame. */
class FixedTrajBodyCentroidJointLeverFactor
    : public ceres::SizedCostFunction<3, 1, 4, 3, 3> {
 public:
  FixedTrajBodyCentroidJointLeverFactor(const BodyTrajectory& traj, double t_bar,
                                        const Eigen::Vector3d& c_L,
                                        const Eigen::Matrix3d& sqrt_info)
      : traj_(traj), t_bar_(t_bar), c_L_(c_L), sqrt_info_(sqrt_info) {}
  ...
  bool Evaluate(double const* const* parameters, double* residuals,
    ...
    const Eigen::Map<const Eigen::Vector3d> L_B(parameters[3]);
    const SE3d T_LW(SO3d(q_LW), t_LW);
    const SO3d R_WB = traj_.rotation_wb(t_eval);
    const Eigen::Vector3d p_WB = traj_.position_wb(t_eval);
    const Eigen::Vector3d p_W = p_WB + R_WB * L_B;
    ...
    const Eigen::Vector3d v_W = WorldPointVelocity(traj_, t_eval, L_B);
    if (jacobians[0]) {
    ...
    if (jacobians[3]) {
      Eigen::Map<Eigen::Matrix<double, 3, 3, Eigen::RowMajor>> J_L(jacobians[3]);
      J_L = sqrt_info_ * (-R_LW * R_WB.matrix());
    }
    return true;
  }
