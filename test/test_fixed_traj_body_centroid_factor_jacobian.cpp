...
TEST(FixedTrajBodyCentroidFactor, JacobianMatchesNumericOver20Trials) {
  ...
  for (int trial = 0; trial < 20; ++trial) {
    ...
    const Eigen::Vector3d p_W =
        ...
        traj.rotation_wb(t_bar - t_d).matrix() * L_B_to_body;
    const Eigen::Vector3d c_L = T_LW * p_W + 0.02 * Eigen::Vector3d::Random();

    clic_calib::FixedTrajBodyCentroidFactor factor(traj, t_bar, c_L, L_B_to_body,
                                                   sqrt_info);

    double t_d_var = t_d;
    std::vector<double*> params = {&t_d_var, q_LW.coeffs().data(), t_LW.data()};
    const std::vector<int> block_sizes = {1, 4, 3};
